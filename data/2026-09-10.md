...
TL;DR: 研究在逐步遮挡与问题相关视觉证据时，VLM 的置信度是否应当单调降低，却经常出现非单调。作者构建基于GQA的证据丢失轨迹，发现原生序列置信度有高比例的单调性违例。训练轻量后验可靠性头，并加入“证据顺序”监督，可小幅降低违例率，但在传统区分正确/错误的指标上优势不显著，说明“证据顺序一致性”与常规可信度判别是不同维度。


<details>
  <summary>Details</summary>
Motivation: 真实应用中，视觉证据退化（遮挡、模糊）时，模型置信度理应随证据减少而下降。如果置信度对证据呈非单调，系统就难以进行可靠的风险控制与人机协作。因此需要刻画并改进VLM在证据丢失过程中的“顺序一致性”。

Method: - 选用冻结的 Qwen2.5-VL-3B-Instruct。
- 基于GQA构建5步、按问题条件定位的关键区域遮挡轨迹，共176条、880种遮挡条件；并设非关键区域等面积对照。
- 定义并测量“证据单调性违例率”(EMVR)：沿遮挡步进，置信度不按证据减少而单调下降的比例；记录相邻步违例。
- 训练一个轻量后验可靠性头（使用冻结隐状态、原生序列置信度与熵作为特征），以BCE为基线；再加入“证据顺序监督”（鼓励随证据减少置信度非增）。
- 评估在遮挡设置与未见的局部高斯模糊上的泛化，并对AUROC、Brier、AURC等传统校准/选择性指标进行比较。

Result: - 原生序列置信度EMVR=0.436，92%轨迹至少有一次相邻违例。
- 关键区完全遮挡使准确率下降28.2个百分点；非关键等面积仅0.6个百分点；成对差异27.6（95%CI [20.0,34.7]）。
- 可靠性头：BCE下EMVR=0.330；加入证据顺序监督降至0.303（配对差-0.027，95%CI [-0.044,-0.010]）。
- 在未见的局部模糊上：EMVR由0.449降至0.402（差-0.0468，95%CI [-0.0739,-0.0199]）。
- AUROC、Brier、AURC两种头间差异统计上不确定；原生置信度在选择性风险排序上仍更强。

Conclusion: 证据顺序一致性是与常规正确性判别不同的维度。通过轻量后验头并使用证据顺序监督可显著但幅度有限地降低非单调违例；然而这并不提升常见的校准/选择性指标，原生置信度在选择性风险上仍占优。应将“证据顺序一致性”作为独立目标，与准确性/校准共同优化，并扩展到更多退化类型与模型。
...
TL;DR: 提出融合RAD-DINO单模态视觉表征与BioViL-T视觉-语言表征的多标签胸片分类框架，在MIMIC-CXR-JPG上实现最优AUROC 0.840、mAP 0.467；早期与混合融合表明两种表征互补，且在各自潜空间精炼后混合融合更优；但仅内部验证，泛化性待评估。


<details>
  <summary>Details</summary>
Motivation: 多标签胸片判别依赖强视觉特征与临床语义知识，单一来源表征往往不足；需要系统探索不同嵌入来源的互补性及更有效的融合策略以提升性能并澄清各自贡献。

Method: 使用两个分支生成嵌入：RAD-DINO提供纯视觉表征，BioViL-T提供视觉-语言表征；分别在潜空间对三个分支（RAD-DINO、BioViL-T及其早期融合）嵌入进行精炼、归一化与跨分支融合（混合/早期融合对比）；在MIMIC-CXR-JPG上对14个标签进行训练与评估，并做消融分析比较不同融合与精炼策略。

Result: 单独使用时RAD-DINO优于BioViL-T；早期融合优于单一来源，显示互补性；最佳模型AUROC均值0.840、mAP 0.467；消融显示在各分支潜空间精炼后，混合融合较早期融合有一致且显著的提升。

Conclusion: 视觉与视觉-语言嵌入互补，提升多标签胸片分类可通过在各分支潜空间精炼后进行混合融合实现；性能提升依赖于每个分支表征质量。当前仅在MIMIC-CXR-JPG内部验证，跨机构泛化仍需外部评估；代码已开源以促进复现与扩展。

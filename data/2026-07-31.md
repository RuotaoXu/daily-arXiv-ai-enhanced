...
TL;DR: 提出一种统计方法度量历史印刷书籍间字体相似性，并用a contrario统计框架评估显著性；在17世纪西班牙戏剧小册子上验证，自动比对Roman与Italic字体，发现并修正排印者归属，推动大规模数字书志学。


<details>
  <summary>Details</summary>
Motivation: 传统书志学依赖专家肉眼比较活字与版式，主观、耗时且难以扩展；需要客观、可量化、可扩展的方法来在大规模文献中识别字体相似性与打印商归属。

Method: 从书页中自动提取字符图像，聚类并对齐以生成字符原型；基于这些原型定义两书之间的“字体距离”；提出a contrario统计框架来判断观测到的距离是否显著，从而给出可操作的结论（如是否同源或高度相似）。

Result: 该方法能够在大规模17世纪西班牙戏剧小册子语料上，自动比较不同书中Roman与Italic字体；专家复核后，新的印刷商归属被发现，既有归属被修订，显示方法在真实场景中的有效性。

Conclusion: 方法为字体相似性提供客观度量与统计显著性解释，补足并加速书志学分析；经实践验证，有潜力将数字书志学扩展到以往难以企及的规模。
...
TL;DR: 提出OVEarth-Bench，一个用于开放词汇地球观测的更全面基准，覆盖更广类别与多样查询形式，统一评估掩码/框零样本定位；评测发现当前方法整体受限，MLLM方法最强，EO专用方法普遍不如通用模型，并给出未来研究与基准建设指引。


<details>
  <summary>Details</summary>
Motivation: 现有开放词汇EO评测覆盖的类别与查询形式狭窄，难以可靠比较方法与推动进展；需要一个更广、更真实且多样化的基准来全面评估模型定位以自然语言描述的地理概念能力。

Method: 构建OVEarth-Bench，扩展两方面：1) 类别广度——分层的大范围类别并包含正/负表达；2) 查询多样性——词汇型、指称型与推理型查询。提供统一零样本协议，支持掩码与框级定位。对广泛的通用与EO特定方法进行系统评测。

Result: 评测表明：(1) 现有方法性能仍受限，且更广的类别覆盖带来更稳定的模型排序；(2) 基于多模态大语言模型（MLLM）的方法总体最强；(3) EO专用方法普遍不及通用模型，鲜有达到最优。

Conclusion: OVEarth-Bench为开放词汇EO提供更现实、全面的评测标准；结果提示应借力MLLM并反思EO专用方法设计，同时强调持续构建多样、高质量、大规模基准对可靠评估的重要性。
...
TL;DR: 提出VETO防编辑伪装与VetoBench基准，针对现代联合注意力图像编辑模型的滥用风险，显著优于现有防御并兼顾图像保真度。


<details>
  <summary>Details</summary>
Motivation: 新一代编辑模型（如FLUX.2）可通过让提示与生成token直接关注参考图像token，实现超越局部修改的重上下文化与对象/身份抽取，极大扩展了潜在滥用面。现有防御主要破坏旧式扩散管线中的参考编码“语义瓶颈”，但被现代“联合注意力”机制绕过，因而需要新的防护方法与更全面的评测。

Method: 提出VETO：一种对源图像施加的细微“反编辑”伪装，专门扰动现代编辑器通过联合注意力读取参考信息的内部机制。同时提出VetoBench评测集，覆盖传统局部编辑与广泛情境重置（recontextualization），系统评估防御效果与保真度权衡。

Result: 在两种当代编辑模型和三个基准上，VETO持续优于现有防御方案，并在保护强度与图像保真度之间取得更佳权衡。

Conclusion: 通过针对联合注意力通道的精细扰动，VETO有效抑制现代图像编辑器的滥用能力；VetoBench填补了对大范围重上下文化编辑的评测空白，验证了方法的全面优势。
...
TL;DR: 从单张插画端到端自动生成可用的Live2D资产：分层RGBA、每层网格与参数驱动的关键姿势位移，实现可编辑、可动画的2D角色。


<details>
  <summary>Details</summary>
Motivation: Live2D广泛用于虚拟主播、手游与交互角色，但建模流程（分层、补遮挡、布网、设关键帧）高度依赖人工，耗时数周；现有生成方法无法一次性产出Live2D所需的结构化资产。

Method: 两阶段：1) 分层扩散生成——依据“器官/部件”级别的Live2D感知分类，将单图分解为有序RGBA层并完成被遮挡区域；2) 仅由每层alpha生成贴合内容的三角网格，并以“所有顶点为token、跨层自注意力”的方式联合预测关键姿位移；位移被分解为有界方向+对数幅值。强调联合而非逐层独立预测以确保整体一致性。

Result: 在50个保留角色上、纯生成（无teacher forcing）条件下，第二阶段达到逐顶点方向余弦0.768（中位数0.828）。网络横向大规模扩展（×112）无显著收益。由alpha导出的网格使服装层可通过文本重贴图而重用原网格与动画。

Conclusion: 首次实现端到端从插画到可用Live2D模型的生成，并提供标准基准Live2D-Bench与包含8,884个模型的有监督语料。方法显著减少人工建模成本，生成结果在一致性与可编辑性上具实用价值。
...
TL;DR: 提出SSCD：用共享离散语义码本跨模态蒸馏，无需成对样本与可比特征；在OCT→眼底与CT→胸片任务上显著提升学生模型宏F1。


<details>
  <summary>Details</summary>
Motivation: 医学影像多模态常不成对且特征空间不兼容，传统实例级蒸馏和直接特征对齐失效，需一种能在无配对数据下稳健传递诊断知识的方法。

Method: 构建跨模态共享的离散语义码本，将每幅图表示为对该码本词汇的分布；通过全局与类别条件的分布对齐实现知识迁移。码本用EMA在线更新，以熵正则保持多样性，并对“死亡”代码重启。推理时丢弃教师与码本，仅保留学生编码器与分类器。

Result: 在两个异质、非成对设定：OCT→眼底的视网膜疾病分类和CT→胸片的肺炎分类中，学生宏F1从64.5提升至70.2、从73.8提升至76.3，均优于所有对比蒸馏基线。

Conclusion: 共享语义码本可在无配对、跨模态条件下有效传递知识，提升廉价模态的诊断性能，并在推理阶段保持部署高效。
...
TL;DR: PanDent提出牙齿级（tooth-level）的OPG多模态评测基准，含9,524张高质量全景片与专家验证结构化标注，并构建与放射学报告一一对应的临床证据对照，用于评估MLLM报告的临床一致性。微调后显著提升定位与诊断一致性，成为评估牙齿级临床推理的严谨基准与数据资源。


<details>
  <summary>Details</summary>
Motivation: 当前MLLM在口腔全景片中缺乏细粒度、可临床信赖的评测基准，导致模型能写通顺报告却难以保证与专家证据一致的临床正确性。需要一个由专家验证、能将结构化证据与自由文本报告严格对齐的资源。

Method: 构建PanDent：收集9,524张高质量OPG；由资深牙医标注牙齿级结构化发现，并由口腔颌面放射科医师复核；依据临床定义的报告逻辑，将结构化证据转化为一致的放射学报告，实现证据-文本的显式对应。以多种MLLM（专有SOTA、通用开源、医学专向）进行评测，并在PanDent上进行微调。

Result: 现有MLLM能生成流畅报告，但在细粒度定位和牙齿级诊断上错误较多，临床一致性不足；在PanDent上微调后，结构-语言一致性、可视定位精度与诊断正确性均显著提升，输出更接近专家解释。

Conclusion: PanDent提供了大规模、临床扎实的OPG牙齿级评测与训练基准，能够严格考察MLLM的临床推理一致性，并作为推动临床可靠牙科AI的重要资源。
...
TL;DR: VideoCoCo用可执行的Blender代码作为“过程级链路”，先生成物理一致的草稿再经视频生成器润色，显著提升物理一致性与基准分数。


<details>
  <summary>Details</summary>
Motivation: 纯文本提示过度压缩，难以隐式推断时空过程；现有CoT/中间态多为不可执行或时间稀疏，无法全面控制与检查动态生成过程。

Method: 提出双引擎框架：1) 编码代理把文本转为可执行Blender程序，显式定义场景与时序演化；2) 模拟引擎执行代码得确定性时空草稿；3) 生成式视频引擎进行基于草稿的编辑以获得写实视频。为适配编辑器，构建VideoCoCo-3K数据集（草稿-指令-目标三元组）以训练/微调。

Result: 在PhyGenBench上从0.475升至0.558，在VBench-2.0上从52.18升至77.88，均为当前最优平均分，显示出显著的物理一致性与可控性优势。

Conclusion: 将可执行代码作为可控、可检查的中间表示，有效分离过程级推理与高保真渲染，可提升文本到视频的物理一致性与可控性。
...
TL;DR: 提出一种单源高效可迁移对抗攻击方法IGME，用链式可微攻击组件与路径平均方向稳定更新，在VOC与Cityscapes上对CNN与Transformer分割模型实现与单源基线相当的迁移性、优于多模型集成的运行时，仅需一个源模型。


<details>
  <summary>Details</summary>
Motivation: 现有分割模型对抗扰动具强迁移性，但评估/生成迁移攻击代价高；常见集成攻击需多替代模型，计算更昂贵且在密集预测上更难。需要一种仅用单源模型、同时兼顾迁移效果与效率的方法。

Method: 将“可迁移攻击”的构造表述为对一串可微攻击组件的链式计算，使昂贵的源模型梯度可在链中共享；为缓解链式组合带来的更新不稳定，引入类似Integrated Gradients的路径平均方向作为经验性稳定启发式，得到算法IGME。

Result: 在Pascal VOC与Cityscapes上，对CNN与Transformer分割模型做跨模型迁移评测；相较单源基线取得有竞争力的迁移性，同时在运行时间上优于需要多模型的集成攻击；仅访问一个源模型即可。

Conclusion: IGME在不增加模型数量的前提下兼顾迁移效果与计算效率，为分割任务的可迁移对抗攻击提供了单源、稳定且高效的替代方案。
...
TL;DR: 提出ProgFormer：一种分层体素空间扩散Transformer，通过粗细两条路径和条件流匹配直接在体素空间预测速度场，从噪声积分生成未来脑MRI；在ADNI/AIBL/OASIS、配对与轨迹设置下优于SOTA。


<details>
  <summary>Details</summary>
Motivation: 纵向脑MRI预测难在整体结构大体稳定而病变进展细微、局部且稀疏；潜变量方法压缩重建易丢信息，体素直推方法用统一通道会被稳定结构主导，淹没细微变化。需要既保持全局一致又敏感于局部进展的模型，并避免自编码器带来的信息瓶颈。

Method: 提出ProgFormer：分层体素空间Diffusion Transformer。- 粗路径：以3D patch tokens进行体积级主预测，建模全脑结构与时序上下文；- 细路径：在每个patch内以粗表示为时空锚点做体素级细化；- 两路径联合通过conditional flow matching在体素空间直接估计速度场；- 通过欧拉积分沿估计速度场从高斯噪声生成未来扫描；端到端，无需单独训练图像自编码器。

Result: 在ADNI、AIBL、OASIS三个基准、配对与轨迹两种设置上，实验显示ProgFormer较多种SOTA方法取得更优性能（文中称favourable）。

Conclusion: 分层粗细路径与体素空间条件流匹配有效兼顾全局稳定结构与局部细微进展，避免潜空间信息损失与统一通道的掩蔽效应，在多基准上表现优越，适合纵向脑MRI预测。
...
TL;DR: 提出Drawing-Recode：从栅格2D CAD图自动生成结构化参数化CAD代码（SPCC），通过图像编码几何、文本模块识别标注，并以跨注意力和注释绑定损失（AGL）将尺寸注释与几何显式对齐，最后由LLM生成代码；在基准与接近工业扫描条件下优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 旧有大量纸质或栅格CAD图在数字化与制造自动化中难以复用；现有方法多依赖矢量图或特定领域，且未显式将尺寸标注与几何关联，限制了参数化3D恢复能力。

Method: 构建Drawing-Recode框架：1) 图像编码器提取几何特征；2) 独立文本识别模块抽取尺寸/注释；3) 通过跨注意力并引入Annotation Grounding Loss（AGL）将注释与几何要素绑定；4) 将融合特征输入LLM，生成结构化参数化CAD代码（SPCC）。

Result: 在实验中显著优于现有基线，并在模拟工业场景的扫描图上保持鲁棒性。

Conclusion: 该方法有效将栅格2D CAD图转化为参数化CAD代码，提升了对旧图纸的数字化能力，助力零件再造与制造流程自动化。
...
TL;DR: 论文研究多图像医学VQA的推理策略，发现简单稳健的“顺序投票”聚合器优于更复杂的重排序方法与固定基线；增大进化搜索预算无助于泛化。


<details>
  <summary>Details</summary>
Motivation: 多图像医学VQA需要在有序影像间整合证据、对答案顺序扰动保持鲁棒，并避免对搜索时反馈过拟合。现有工作往往当作提示长度问题或依赖复杂代理策略，缺乏可复现的对比与泛化检验。

Method: 在MedFrameQA上，构建可控实验：比较5种推理时代理策略，统一使用高预算ShinkaEvolve配置优化；在固定可复现的数据划分上评估（1331进化、665保留、855最终测试）；进行5次独立重复运行，并用配对自举法检验显著性；还比较了50代与100代进化预算对泛化的影响。

Result: 最强方法为“order-vote”顺序聚合投票，最终测试准确率57.89±0.65%，显著优于固定基线52.73±0.42%和更复杂但脆弱的order-rerank 55.79±0.43%；将进化预算从50增至100代，最终测试从57.89%降至56.02%，虽保留集略有上升但无泛化收益。

Conclusion: 在多图像医学推理中，选择正确的代理决策规则比扩大优化搜索预算更关键；简单且鲁棒的顺序投票优于复杂重排序，避免对搜索阶段噪声过拟合。
...
TL;DR: 论文在MedFrameQA多帧医疗VQA上对比多种复杂适配策略，发现与最终答案目标强对齐的“直接答案SFT”（decoder-only）是最稳健、最强的适配家族，优于控制器、重排序、混合监督与续写等复杂方法；校准可修复置信度而不损伤准确率，并可迁移到其他骨干。


<details>
  <summary>Details</summary>
Motivation: 现有多帧医疗VQA常趋向引入复杂组件（控制器式推理、定位感知重排、硬负样本混合、分阶段续写），但其真实增益与稳健性在受控评测下未被系统验证。作者怀疑：也许与评测目标（最终答案）更对齐的简单方法，反而在稳健性上更优。

Method: 在MedFrameQA基准上搭建统一受控评测：固定数据划分、等算力预算、重复随机种子与后验校准；比较多类方法——控制器式方法、脚手架演化、静态混合监督、重续写变体、以及仅用最终答案的直接SFT（decoder-only）。主干以MedGemma-1.5-4B为主，并测试向Qwen2.5-VL-3B的可迁移性；评估包含报告准确率、种子方差与校准质量。

Result: 直接答案SFT在MedGemma-1.5-4B上取得显著高于冻结基线的保留集报告准确率，且在重复种子、等预算、匹配控制下表现最稳定；后验校准显著改善置信度估计而不损失准确率；该策略可稳定迁移到Qwen2.5-VL-3B。复杂附加机制未能在稳健性与总体表现上超越。

Conclusion: 与评测目标强对齐的最小化方案（decoder-only直接答案SFT）构成当前在MedFrameQA上最稳健的适配家族，应将研究重心从架构复杂化转向稳健优化与目标对齐；所给出的方法亦提供一个强有力、可迁移的基线。
...
TL;DR: 提出 ZMIS-SAM，将SAM与小波特征增强结合，在浮游动物显微图实例分割上达SOTA，并具有跨域泛化能力。


<details>
  <summary>Details</summary>
Motivation: 通用SAM在显微图像、尤其是浮游动物实例分割上表现受限：缺乏领域知识导致分类不准、细长附肢分割中断、边界不完整。需要一个既保留SAM泛化、又注入领域与高频细节建模的方案。

Method: 在SAM基础上引入三项改进：1) ZM-ViT：通过两个轻量适配器增强对形态与强度分布的建模；2) 邻域特征聚合模块（NFAM）：融合通用与领域特征，提升半透明细长附肢的连续分割；3) 基于小波的多尺度多方向特征增强（WM2FE）：恢复高频细节以完善边界。整体形成端到端的实例分割框架 ZMIS-SAM。

Result: 在自建浮游动物数据集上取得SOTA实例分割表现；在多个人工公开跨域数据集上也显示出较强的泛化与鲁棒性。

Conclusion: 通过在SAM中注入领域特化建模与小波高频增强，ZMIS-SAM显著改善了显微图的实例分割质量，尤其在细长附肢连续性与边界完整性方面，并保持良好跨域泛化。
...
TL;DR: MeshFM 提出一种高效前馈框架，将2D视觉大模型的特征蒸馏到3D中，通过两阶段训练在无3D标注下学习可直接用于多任务的3D特征，效果媲美有3D监督的方法且对极端旋转鲁棒。


<details>
  <summary>Details</summary>
Motivation: 现有3D特征学习常依赖昂贵的3D标注或推理时优化，泛化与效率受限；而2D基础模型蕴含丰富视觉先验，如何将其有效迁移到3D以支持下游任务仍是难题。

Method: 两阶段：1) 仅用2D特征监督在三维中优化一个稠密特征场（feature field）；2) 训练一个前馈网络去回归该三维特征场，实现推理时无优化的直接预测。同时在训练中设计使模型对物体极端旋转具有鲁棒性。

Result: 在零3D标注与无任务特定微调条件下，所学3D特征可直接用于部件分割、稠密对应、网格变形等任务，整体性能与显式使用3D监督的方法相当。

Conclusion: 通过将2D基础模型特征蒸馏为3D特征并训练前馈回归器，MeshFM在无需3D标注与推理优化的前提下实现高效、通用且对姿态鲁棒的3D特征表示，可广泛支持多种下游3D任务。
...
TL;DR: 提出MPIE-Bench与MPIE-Eval，用多人物体接触场景评测图像编辑模型的解剖完整性与交互接触几何；发现现有VLM评审高分但掩盖明显错误，所提指标更贴近人类判断。


<details>
  <summary>Details</summary>
Motivation: 单人高保真合成已成熟，但多名特定人物的接触互动（拥抱、抱起、扭打等）仍频现解剖与几何错误；现有评测与VLM判官在“互动”维度上易饱和，无法反映明显瑕疵，亟需更可靠的客观评估基准与指标。

Method: 构建MPIE-Bench：来自视频挖掘的2,500条编辑三元组，覆盖405个场景、14类互动与4个接触密度等级（C0–C3）。提出MPIE-Eval：基于冻结的公开多人网格重建，定义两条新评估轴——Anatomy与Interaction。Anatomy检验生成图中每个人形质量是否由完整可重建人体网格解释；Interaction检验人体间的穿插与表面距离是否与指令所要求的接触相匹配。对10个编辑器进行评测。

Result: 在两种不同模型上，网格Anatomy最高约0.65，网格Interaction最高约0.72，未有单一编辑器在两轴上同时领先；而VLM清单式打分却普遍>0.95。五位人工评审研究显示，两条新轴与人类判断的相关性显著高于零样本VLM评审，且在各类权重与阈值消融下排名稳定。

Conclusion: 多人物接触编辑需专注于解剖完整性与接触几何的量化评估；MPIE-Bench与MPIE-Eval提供更可靠、人类一致性更高的测评框架，揭示VLM打分的虚高与盲点，为后续方法改进与公平比较奠定基准。
...
TL;DR: 提出MedXplore用于医学影像的一般化类别发现，在感知与决策两层优化，显著提升全体准确率（+8.5%），并极大降低旧类偏置（Kvasir假旧率14.5%→0.8%）。


<details>
  <summary>Details</summary>
Motivation: 医学影像DL常依赖大规模标注与封闭世界假设，不符合临床现实；自然图像GCD发展快但在医学领域匮乏，需能在部分有标注和大量无标注且含未知新类的场景下稳定发现新类别并避免旧类偏置。

Method: 统一框架MedXplore：1) 感知层FAAC，从频域视角进行可学习全频滤波与全局-局部能量对比激活，突出相对全局上下文的局部异常信号，并提供稳定语义锚用于patch一致性学习；2) 决策层ACAM，根据语义难度与特征置信度自适应调节角度间隔，协调类内紧致与类间分离，缓解旧类偏置。

Result: 在多基准上All准确率平均提升8.5%，在Kvasir数据集显著减少“将新类误判为旧类”的错误（14.50%降至0.80%），在旧-新高度混淆时仍具鲁棒性。

Conclusion: MedXplore在医学GCD中实现可靠、无偏的新类发现：频域感知增强+自适应角度边距共同提升病灶敏感表示并降低旧类偏置，优于现有最强方法，具临床应用潜力。
...
TL;DR: 提出BlindPSNR：首个针对低光增强参数选择的无参考PSNR预测模型，能高准确率选出最优参数并显著降低遗憾值。


<details>
  <summary>Details</summary>
Motivation: LLIE常需调参，但参数通常固定，跨场景泛化差；人工挑选配置代价高。理想自动准则是PSNR，但需参考真值，实务中缺失。现有NR-IQA关注主观感知而非信号保真，且在作者基准上完全失效，亟需能在无参考条件下预测PSNR以实现自动参数选择的方法。

Method: 利用成对数据可解析计算PSNR作为精确监督，训练一个轻量级无参考网络BlindPSNR：将增强图与低光输入通过窗口化交叉注意力进行融合；采用异方差回归输出PSNR及其不确定性。与纯标量回归基线对比，结构更能捕获低光退化与增强结果之间的保真关系。

Result: 在参数选择任务上，标量回归基线top-1准确率54.4%，遗憾1.62 dB；BlindPSNR将top-1提高到89.5%，遗憾降至0.026 dB；在未见数据集上相关性SRCC为0.61–0.67，显示一定泛化能力。

Conclusion: BlindPSNR无需参考真值即可有效预测增强结果的PSNR，实现跨场景的自动参数选择，显著优于NR-IQA与标量回归基线，并具备一定泛化性。
...
TL;DR: 4DHumanDiff 是一个端到端的文本到4D高斯溅射(4DGS)的人体生成扩散框架，直接输出可渲染的动态人资产；无需先生成视频或逐场景重建，因而在多视角与时间一致性更好、速度更快。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常先合成单/多视角视频再拟合4D表示，计算昂贵、易导致几何不完整与视角不一致。需要一种能直接在结构化4D表示空间中生成、兼顾时空一致性且高效的方案。

Method: 以4DGS为目标表示，构建带时间注意力的3D U-Net扩散模型，端到端从文本生成动态人体；配套构建6万对文本-4DGS数据；引入2D正则化提升渲染质量，并通过免训练的4D插值增强运动平滑。

Result: 在约一分钟内生成一致的360°动态人体；较现有方法显著提升时间与多视角一致性；推理时间减少超10倍。

Conclusion: 直接在4DGS空间进行扩散生成能高效产出时空一致的动态人体资产，是替代“先视频再重建”流程的有效方案，并在质量与速度上取得显著优势。
...
TL;DR: 提出MMOOC基准，系统评估多模态大模型在“拒答真正越界问题”和“在上下文移位但仍可回答的问题中保持作答”的能力；结果显示现有模型难以兼顾拒答与作答，后训练有助提升鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM在上下文不完美或分布偏移时易失效。理想模型应：对主体层面的越界(OOC)问题拒答，但对非主体层面的“移位但仍在上下文内”(Shifted IC)问题作答。现有基准多聚焦OOC或纯视觉不可答，忽视可答的Shifted IC并且OOC类型覆盖有限，因此需要一个覆盖更全面移位类型、同时评估“拒答与作答平衡”的基准。

Method: 构建MMOOC大规模基准：包含41K+图文对，覆盖三种问答格式、八类上下文移位、六种视觉场景；同时包含可答的Shifted IC与不可答的OOC样例。通过MLLM过滤加人工校验保证质量。评估指标含Accuracy与Refusal Rate，并引入LLM-as-a-Judge评估推理正确性。对多种现有MLLM进行实验与失效模式分析，并测试后训练(post-training)对鲁棒性的影响。

Result: 多种主流MLLM在移位场景下难以在“该答时答、该拒时拒”之间取得良好平衡；在MMOOC上准确率与恰当拒答率均不理想。LLM-as-a-Judge显示推理链经常错误或不一致。后训练策略可明显提升鲁棒性与总体表现。

Conclusion: MMOOC填补了评测空白，为系统衡量MLLM在移位上下文下的拒答与作答鲁棒性提供标准化基准；当前模型仍存在显著短板，针对性后训练可改进。数据与基准将公开。
...
TL;DR: 提出FedPAIE，一个在不集中原始照片/评分的前提下进行个性化美学图像增强的联邦学习框架，结合个性化美学评分器与轻量CLUT增强器，并通过正则化与约束避免过度优化，达到在移动端高效、自然的用户偏好色彩调整。


<details>
  <summary>Details</summary>
Motivation: 个性化图像增强需要反映个人审美，但训练通常依赖私密照片与评分，难以集中收集；用户侧反馈稀疏异质，设备算力受限，且需保证增强结果自然、保真，避免为代理分数过拟合。

Method: 在联邦框架中：1) 训练轻量双线索（dual-cue）美学评分器；2) 在本地小支持集上标定为个性化评分器并冻结；3) 用冻结的个性化评分器指导轻量CLUT增强器在非配对本地照片上自适应；4) 通过保真约束与excess-gap惩罚，限制对代理分数的过优化并保持内容与自然外观。参数规模：评分器更新≤0.787M，增强器自适应更新0.265M，推理仅保留0.293M参数的个性化增强器。

Result: 在MIT-Adobe FiveK与Flickr-AES上展示了开放世界的个性化效果，在用户偏好与图像保真之间取得有利平衡。

Conclusion: FedPAIE将去中心化偏好学习与高效个性化图像变换相连接，无需成对用户润色数据即可实现用户自适应的色彩分级。
...
TL;DR: 提出WEP方法：直接利用MLLM白盒前填路径，对“封闭式视觉回答”的风险进行推断时检测；通过层级估计视觉证据的正负贡献，构建可解释证据路线并与置信度融合，从而更好地区分应信任、复核或转路的答案。


<details>
  <summary>Details</summary>
Motivation: 仅用候选置信度（如边际）不足以反映答案背后“来自何处的视觉读出”及其分布，导致在部署时难以可靠判定答案是否可信或需升级；现有方法常依赖扰动、外部验证器或额外计算，缺乏高效、可解释、可在白盒条件下直接利用的风险指标。

Method: 在推理时复用产生答案的同一白盒prefill前向路径：逐层估计对预测候选的视觉贡献（支持或反驳）；将贡献汇总为两类可解释“路线族”：(1) 与问题相关的证据溯源；(2) 带符号的证据集中度。通过嵌套分组验证选择更可靠的路线族及稀疏top-k路线组合（portfolio），并与候选置信度融合形成最终风险分数。无需图像扰动、解码策略改变、反向传播或外部验证器。

Result: 在3个MLLM与4个二元回答基准上，WEP将mean error AP提升0.134；12个模型-数据集组合均取得正增益，其中10对在图像簇自举区间内显著为正。

Conclusion: WEP为白盒封闭答案场景提供低开销、可解释的风险检测机制，能稳定改进错误识别与路由决策；需一小段有标签校准数据，主要适用于可访问模型内部读出的系统。
...
TL;DR: 提出一个带榫卯形状拼接口的拼图基准（非矩形切割），通过几何与视觉双重约束消除重纹理带来的多解；评测显示零样本VLM几乎不具备几何推理，微调在小规模有效，但随规模增大性能陡崖式坠落，凸显可扩展几何推理仍是开放难题。


<details>
  <summary>Details</summary>
Motivation: 现有拼图基准使用规则矩形切割，遇到重复纹理时解空间多解且几何约束弱，难以准确评估模型的真实几何推理能力。需要一个能提供强局部兼容性、同时结合视觉内容、并具有明确唯一解的基准，以检验VLM在可扩展几何推理上的能力。

Method: 构建包含“榫-槽”互锁边缘的拼图数据集（4×4到16×16四种密度，约95K实例），其几何形状带来强局部匹配约束，从而与视觉线索联合决定唯一解。用多种前沿零样本VLM与经监督微调的模型在不同网格规模上系统评测。

Result: 零样本VLM在4×4上多数接近随机，只有GPT-5.5略高于随机；监督微调可在4×4上达到>97%。但随着网格增大，所有模型出现“规模悬崖”：GPT-5.5在8×8从约70%跌至近随机，微调模型在12×12也低于5%。

Conclusion: 当前VLM架构难以在拼图规模增大时持续满足全局几何约束，表明可扩展的几何推理仍是待解挑战；该基准为评估和推动此方向提供了标准化测试平台。

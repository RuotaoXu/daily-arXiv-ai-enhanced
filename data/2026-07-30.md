...
TL;DR: 提出KDA：用LLMs将动作标签分解为原子动作，并通过知识注入与知识解耦模块提升多标签细粒度动作识别，达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 复杂场景中并发、细粒度动作使整体式表示难以刻画内部结构与微妙交互；现有提示/视觉/结构方法语义引导不足、粒度偏粗，需要显式的细粒度语义来提升表征与解耦。

Method: 1) 用大语言模型将高层动作标签分解为具备时空指向性的原子动作；2) 知识注入模块KIM将原子动作知识融合到视频特征中；3) 在增强表征上，知识解耦模块KDM进一步解耦原子动作成分，生成更精确的语义引导用于动作解耦；4) 设计知识解耦损失KD Loss，促使KDM内部的知识成分更清晰可分。KIM与KDM可插拔于其他方法。

Result: 在多标签动作识别基准上取得SOTA，并显著提升特征可判别性；模块具有良好通用性，嵌入他法亦有收益。

Conclusion: 细粒度语义知识驱动的原子动作分解与注入-解耦两阶段框架，有效提升多动作场景下的表征与解耦能力，带来SOTA性能与较强可迁移性。
...
TL;DR: 利用社交网络中的“野外”单人图像，融合多模态并采用单/多任务深度学习来回归身高、体重与BMI；并发布6105张全身标注数据集；实验证明全身图像优于半身与人脸。


<details>
  <summary>Details</summary>
Motivation: BMI与身高、体重是重要健康与生活指标，能辅助疾病风险预测与寿命估计。自动从自然场景单图像估计BMI很有价值，但受姿态、相机几何、外观差异与杂乱背景等因素影响，问题极具挑战；且缺乏公开全身数据集。

Method: 构建含6105张“野外”全/半身与多视角图像的数据集，带身高、体重、BMI标注。采用深度神经网络比较单任务与多任务学习，输入多模态：RGB、深度图（估计）、姿态亲和图、边缘图；试验多种CNN骨干（VGG、DenseNet、ResNet），并比较使用全身、半身、仅人脸三种裁剪。

Result: 在广泛实验中，全身图像输入的预测误差最低，优于半身与人脸；多模态与多任务策略带来性能提升（相对基线CNN）。

Conclusion: 在自然场景中从单图像估计BMI/身高/体重是可行的；高质量全身信息尤为关键。所发布的数据集可作为该任务的基准，方法为后续研究提供了实现与比较基础。
...
TL;DR: TraceCLIP 是一个无需训练的方法，通过解析 CLIP 中 CLS 注意力输出的补丁特定贡献，恢复潜在的补丁级语义，并以此校准最终层的补丁亲和度，从而提升零样本密集视觉-语言任务（尤其语义分割）的性能。


<details>
  <summary>Details</summary>
Motivation: CLIP 的对比学习目标在图像级对齐文本与全局 CLS 表征，缺乏对局部视觉-语言对应关系的直接约束，导致在对象定位、区域识别、开放词表分割等密集任务上存在局限。现有方法多需额外监督、外部模型或任务特化训练；训练自由方法则通常直接从现有补丁特征恢复密集响应，却未探究局部语义在 CLIP 内部何处最易获取。

Method: 提出 TraceCLIP：不训练、直接在现有 CLIP 上运行。其关键是将 CLS 注意力输出分解，隔离每个补丁对 CLS 的特定贡献，作为“贡献衍生”的补丁级语义证据；随后将这些响应转换为“语义测地”拓扑门，去校准最终层补丁亲和度，用于重建更准确的密集特征以支持零样本分割等任务。

Result: 在 8 个零样本语义分割基准上，相比最强的既有训练自由方法，平均 mIoU 提升 1.3–4.5 个点，且跨不同主干与背景设置均有效；无需额外训练、外部视觉基础模型或区域级监督。诊断实验显示该贡献特征具备强局部语义判别力与文本条件的空间对齐能力。

Conclusion: 即便 CLIP 以全局对齐为目标，其内部构造中仍保留可访问的空间局部语义。通过追踪 CLS 注意力中的补丁贡献并据此校准补丁亲和度，可在无需训练的前提下显著改进密集视觉-语言理解，提示进一步挖掘预训练模型内部可显式化的局部语义潜力。
...
TL;DR: 提出DVPSFormer：一个统一的在线DVPS框架，利用显式场景离散化与离散到连续深度头高效耦合语义与几何，并以在线多数投票提升时序分类，一次前向实现实时4D场景理解并在Cityscapes-DVPS与SemKITTI-DVPS上SOTA。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶/机器人需要同时获得深度、语义与实例轨迹以实现安全在线感知。现有DVPS方法多为多阶段、离线跟踪或高计算开销，难以实时部署。

Method: - 统一在线架构DVPSFormer。
- 显式场景离散化（ESD）：用分割queries分别表示前景与背景区域，将场景离散化。
- D2C（离散到连续）深度头：基于上述离散表示一次性回归度量深度，实现语义-几何的紧耦合与低延迟。
- 在线多数投票（OMV）：利用时序一致性在跟踪过程中修正/稳定类别判定。

Result: 在Cityscapes-DVPS与SemKITTI-DVPS基准上取得新的SOTA，同时具备在线、低延迟特性；代码与模型开源。

Conclusion: DVPSFormer以ESD和D2C深度头实现高效联合学习，并用OMV增强时序分类稳定性，提供了适合实时机器人感知的端到端在线DVPS解决方案。
...
TL;DR: 提出一种混合计算机视觉方法，从图像量化裸露皮肤面积用于皮肤接触暴露评估；在170张室内涂刷图像上，先用Mask R-CNN定位人体并去背景，再用颜色分割提取皮肤，裸露皮肤/人体像素比与人工估计约80%一致，方法可扩展至部位识别、PPE检测与视频分析。


<details>
  <summary>Details</summary>
Motivation: 传统职业卫生中皮肤暴露评估依赖人工观察或现场测量，效率低、主观性强且难以规模化；社交媒体与作业现场图像丰富，若能自动从图像提取暴露信息，将显著提升大规模、低成本的暴露监测能力。

Method: 构建两阶段流水线：1) 采用Mask R-CNN在涂装场景图像中检测并分割人体，抑制背景干扰；2) 在人体掩膜内应用基于颜色的皮肤分割算法，计算裸露皮肤像素占人体像素比例；与人工标注/估计进行一致性评估。

Result: 在170张室内涂刷图像上，自动计算的裸露皮肤/人体像素比与人工估计达到约80%的吻合度，表明方法具有可用性与一定鲁棒性。

Conclusion: 该混合方法为从静态图像提取半定量皮肤暴露信息提供了可扩展方案；未来可通过引入身体部位识别、个体防护装备(PPE)检测及视频时序分析提升精度、细化暴露部位并实现时变暴露评估。
...
TL;DR: 提出Rad-JEPA 3D：一种用于CT体数据的自监督联合嵌入预测框架，结合H-Mamba混合编码器与隐藏状态正交正则(HSOR)，在无需大模型规模下获得更强的三维空间表示与SOTA空间推理表现。


<details>
  <summary>Details</summary>
Motivation: 现有三维医学影像自监督预训练难以保留粗粒度空间与几何结构，导致在器官解耦、异常检测与与语言模型联合的空间理解上受限；需要一种能更好捕获体数据跨切面与体素级结构的预训练方法。

Method: 采用Rad-JEPA 3D：给定被遮挡视图，预测完整CT的潜特征（联合嵌入预测）。核心为H-Mamba混合编码器：一支Mamba状态空间分支建模切片序列的跨层连续性，另一支分组查询注意力捕获跨平面空间上下文，并通过轻量逐token路由融合；提出HSOR层级正则，对齐师生隐藏状态并鼓励正交以减少特征冗余，提升中间表示质量。

Result: 在约12万例CT上预训练，参数仅4.0B，却在封闭式VQA上达到与SOTA相当的成绩，并在Spatial-Med基准上获得最佳平均空间推理分数；消融显示混合块与HSOR带来互补收益，所诱导的空间结构在体数据推理中可替代单纯增大语言模型规模。

Conclusion: 通过JEPA目标与H-Mamba+HSOR的设计，模型能学习到更一致且判别性的三维体表示，提升器官识别和空间推理等下游任务，在较小模型规模下实现SOTA级性能，强调结构归纳对体数据理解的重要性。
...
TL;DR: 提出WildShadowRemover：基于视频扩散模型的LoRA微调方法，实现野外场景视频去阴影，结合细节注入、频域调制和单目深度先验，并发布大规模合成视频基准WildShadow，显著提升去阴影质量与时序一致性与泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现实世界视频存在复杂光照和多样阴影形态，现有方法与数据不足，导致在“野外”场景中去阴影效果差、时间一致性弱。需要一种能利用强生成先验、保持细节并稳健泛化的方案与数据基准。

Method: 在预训练视频扩散模型上进行LoRA微调；冻结VAE解码器并加入细节注入模块；设计由阴影掩码引导的频率分解调制模块，有选择地恢复高频纹理、抑制阴影伪影；引入Depth Anything 3的单目深度先验，提供几何感知引导；同时构建WildShadow大规模成对视频去阴影数据集与评测基准。

Result: 在广泛实验中，相比现有方法，该方法在去阴影质量与时间一致性上更优，生成具有更好视觉质量和跨复杂真实场景的强泛化的、时序连贯的无阴影视频。

Conclusion: 利用视频扩散模型的生成先验并结合细节注入、频域调制与深度先验，可在野外复杂条件下实现稳健的视频去阴影；配套数据集WildShadow推动评测与研究。
...
TL;DR: 提出一种将联邦学习与物理约束相结合的联邦物理信息神经网络，用于从MRI推断脑肿瘤相关组织位移与刚度变化，在不共享原始数据的前提下取得比集中式基线更高的整体准确率与AUC，并生成物理一致的形变场。


<details>
  <summary>Details</summary>
Motivation: 脑肿瘤会改变脑组织力学特性，但常规静态影像无法刻画随时间的生长、位移与刚度变化。数据隐私法规（GDPR/HIPAA）又限制多中心数据集中训练，且跨机构泛化性差。需要一种既满足隐私合规又能嵌入生物力学先验、提升泛化和可解释性的学习框架。

Method: 构建联邦物理信息神经网络：三家模拟临床站点各自用病人MRI训练本地网络，损失函数包含基于线性弹性方程的物理项；使用FedAvg在100轮中仅聚合模型权重，不传输原始数据；评估分类性能（总体准确率、各类AUC）与预测的位移场物理一致性（平滑、无散）。

Result: 联邦模型总体准确率91.4%（优于集中式非联邦基线90.0%），平均AUC 0.985；垂体瘤准确率从85.6%提升至94.5%。产生的位移场平滑且发散近零，符合预期组织形变。

Conclusion: 联邦学习可与基于线性弹性物理约束的损失有效结合，在保护隐私的同时，不但不损失性能，反而在总体与某些肿瘤类别上超过集中式基线，并提供物理一致的形变估计，表明其在跨机构神经肿瘤分析中的可行性与优势。
...
TL;DR: 提出LACA模块，面向双手动作分割，显式估计左右手特征流的时间偏移分布，并含“空”状态以在不匹配时抑制跨手信息传递；无额外标注、以兼容性目标自动监督。集成到Polyphony在HA-ViD与ATTACH上显著提升F1与边界F1，参数开销极小，并给出仅用历史信息的LACA-C实现出色的时效与精度。


<details>
  <summary>Details</summary>
Motivation: 现有双手动作分割通常在相同时间索引融合左右手特征，忽略手间动作转移存在非零且可变的时间滞后，导致跨手信息对齐不准、边界定位与分割精度受限。

Method: 提出Lag-Aware Cross-Hand Alignment (LACA)：1) 学习左右手之间方向性的时间偏移分布；2) 利用估计的偏移从对侧手检索并聚合匹配特征；3) 设定可学习的“空”状态以在无兼容转移时抑制跨手传递；4) 利用由帧级标注自动生成的兼容性监督，无需额外标注。并提出未来无关的变体LACA-C，仅使用当前与历史观测完成对齐与推理。

Result: 注释分析显示存在大量非零跨手匹配（HA-ViD 44.7%，ATTACH 48.9%），显著高于时移对照（18.6%、21.3%）。集成到Polyphony后：HA-ViD上两手mean F1@50由40.4→42.5、边界F1由56.5→59.6；ATTACH上mean F1@50由19.9→21.8、边界F1由44.7→47.9；仅增加约0.0086M参数。LACA-C在ATTACH上达到83.6%转移线索召回、中位可用延迟233ms、每分钟0.72个误触发、分割阶段224.9 FPS（当前位置预测）。

Conclusion: 显式建模跨手时间对齐能在极小参数开销下稳健提升双手动作分割与边界定位；LACA与未来无关的LACA-C兼顾准确性与实时性，支持无需未来帧的及时感知。
...
TL;DR: 论文提出BG-REAL基准，用于检测与定位图像背景操纵，强调真实数据锚定与误报诊断；基线在重编码真实性图像上普遍存在高误报，暴露共享捷径风险；提供构建管线、评估协议与复现实验。


<details>
  <summary>Details</summary>
Motivation: 现有图像取证评测多偏向前景目标的复制-移动、拼接或通用合成编辑，对“背景被改而前景未改”的真实场景关注不足，导致模型在实用场景下泛化欠佳，且对重编码等非操纵伪迹易产生误报。作者希望提供一个以背景操作为中心、真实数据锚定的基准与诊断方法，补足通用操纵定位基准的空白。

Method: 构建BG-REAL数据与评测框架：从Open Images V7实例分割出发，组织约1,200源组共7,000样本（其中6,000为公共数据锚定、1,000为合成对照），覆盖六类编辑及匹配的真实对照；提供源组划分、掩膜与泄漏质检、599条人工质检记录；实现三套外部基线（TruFor、MVSS-Net、HiFi-Net）与五种随机种子评估；引入“匹配真实对照诊断”在固定阈值下测量重编码真实图像被误判为操纵的频率。

Result: 基线在再编码真实性图像上的误报高：最低的TruFor误报率为0.57，一些较弱或利用掩膜信息的基线达到1.00，显示重编码伪迹是各基线共享的捷径风险；给出整体精度与细粒度诊断结果、并随附可复现实验材料。

Conclusion: BG-REAL是面向背景操纵检测/定位的补充性基准，而非通用或纯真实数据基准；其诊断显示现有方法易将重编码伪迹当作操纵信号，提示未来模型需抑制编码伪迹敏感性并提升对真实背景编辑的鲁棒与可解释检测。
...
TL;DR: 提出FORGE-SIM：从稀疏已定姿RGB图像直接重建多片段B样条B-rep，生成紧凑、光滑、封闭且可仿真的几何体，并将观测场（热、语义）投影到同一样条基上，实现开箱即用的数值仿真。


<details>
  <summary>Details</summary>
Motivation: 图像重建虽能得高视觉细节，但提取的网格/隐式表面通常非封闭、不光滑、拓扑噪声多，不适合CAD与数值仿真（需要显式、闭合、光滑、可参数化的几何）。缺乏从图像直达仿真就绪几何的统一流程，阻碍仿真驱动设计与数字孪生。

Method: 直接以多片段B样条边界表示（multi-patch B-spline B-rep）为优化变量，以稀疏定姿RGB图像的重投影误差等观测约束优化控制点与拓扑；保证几何的光滑与水密性；并提出将图像导出的场（如温度、语义）在同一B样条基上投影/拟合，从而与几何同源表示，兼容CAD/CAE管线。

Result: 在多种对象上得到紧凑、光滑、水密的B-rep模型；这些模型直接用于热传导仿真与模态分析，表现出足够的数值稳定性与精度，同时保持较低参数量与可编辑性。

Conclusion: FORGE-SIM将图像重建与仿真就绪建模统一为单一优化框架，打通计算机视觉与数值分析的壁垒，支持仿真驱动设计、检测与数字孪生的新工作流。
...
TL;DR: LumaGuide提出一种无需再训练的扩散模型采样阶段分布塑形方法，通过可微能量引导，让生成结果的特征分布匹配目标分布，实现HDR等可控生成。


<details>
  <summary>Details</summary>
Motivation: 预训练扩散模型受训练数据统计偏差限制，难以产生高动态范围（HDR）等超出原分布的内容；现有方法多需修改或再训练模型，成本高、泛化差，缺乏在采样时直接控制输出分布的机制。

Method: 提出LumaGuide：在不改动模型参数的前提下，于采样过程中加入可微的能量函数，对生成样本施加分布匹配约束。具体在HDR场景中，于感知均匀的PQ空间对亮度直方图进行对齐，引导扩散过程使输出亮度分布匹配目标；支持通过数据驱动预设、参考图像或文本驱动预测器来定义目标分布，并可在视频中加入时间一致性约束。

Result: 对齐亮度直方图即可诱导出HDR一致的生成行为：高光连贯、阴影细节保留，同时保持语义一致性；方法可扩展到多种目标分布设定以及视频生成，并在不再训练的前提下实现稳定的可控性。

Conclusion: 直接在采样时塑形输出分布，通过能量引导即可实现可控生成，避免再训练成本；在HDR任务中验证了其有效性与通用性，并展示了向更广泛条件与视频时序一致性的自然扩展。
...
TL;DR: 面向风机防撞的猛禽识别，扩充数据与教师模型再微调带来主要收益；蒸馏与换教师版本对集成效果无显著提升；轻量学生集成在边缘端实现高召回与高吞吐。


<details>
  <summary>Details</summary>
Motivation: 风电场需要在边缘设备上实时识别特定猛禽以触发避让/停机，但现有高精度模型（如DINOv2-L）体量大、延迟高，且物种间相似度高导致混淆（特别是白尾海雕与虎头海雕）。需要在不牺牲准确性的情况下实现轻量化与可靠部署，并缓解数据来源/视频切片带来的泄漏偏差。

Method: 以DINOv2-L为教师，训练三种轻量学生（MobileNetV4、ViT-S、EfficientNet-B0），并采用三学生集成。通过视频帧抽取得到更大规模的数据集（12,519张），显著增加虎头海雕样本。采用“组划分”（按视频与来源图像层面分组）以降低源泄漏。进行教师再微调，并比较CE-only与蒸馏、以及DINOv2-L与DINOv3-L教师的影响；在Jetson Orin Nano上用TensorRT FP16部署并评估吞吐与与FP32一致性。

Result: 三学生集成在组划分下五种蒸馏种子平均macro recall为0.935±0.004；在常规图像级划分下为0.955，保留教师97.5%的macro recall，参数量约为教师的1/8。对白尾海雕的召回在独立的1,258张子集上最高提升38.6个百分点，其被误分为虎头海雕的错误占比由61%降至15%。EfficientNet-B0在Orin Nano上TensorRT FP16推理为3.19 ms/图（313张/秒），与FP32的argmax一致率99.95%。在五种子对照中，蒸馏（相对仅CE）与更换DINOv3-L教师均无明确的集成层面提升；主要收益来自数据集扩充与教师再微调。

Conclusion: 针对边缘端风机防撞，轻量学生集成在严格分组划分下实现接近教师的高召回与极高推理效率。性能提升的关键是数据扩充与教师再微调，而非蒸馏技巧或更大/更新教师。部署上，FP16 TensorRT可在低功耗设备上提供百帧级吞吐且与FP32高度一致。
...
TL;DR: 用头颈部癌症术前CT的“CT Foundation”模态基础模型提取特征，经MLP预测远处转移风险，AUC=0.791，优于传统放射组学(0.772)和常规深度学习特征(0.753)，接近二者特征融合模型(0.794)。基础模型特征无需分割与繁琐预处理，更可扩展。


<details>
  <summary>Details</summary>
Motivation: 现有HNC远处转移预测多依赖肿瘤分割与先验ROI，流程耗时、需专家参与且存在主观性。近期模态专属的医学影像基础模型可作为通用特征提取器，有望减少标注依赖并提升下游任务表现，亟需系统评估其在HNC DM风险预测中的效果与可替代性。

Method: 使用RADCURE数据集2327例术前CT。构建三类特征：传统放射组学、常规深度学习特征、CT Foundation模型嵌入；分别输入多层感知机(MLP)二分类预测DM风险，并与放射组学+深度学习特征融合模型对比。评估指标为ROC AUC。

Result: CT Foundation嵌入的MLP达AUC 0.791，优于放射组学(0.772)与深度学习特征(0.753)，与放射组学+深度学习融合模型AUC 0.794相近。

Conclusion: 以模态基础模型提取的特征可在无需精细ROI分割和大量标注的前提下，达到甚至接近多源融合的性能，降低专家依赖与预处理成本，具备更高可及性与可扩展性，是传统放射组学的有前景替代方案。
...
TL;DR: 提出HeteroPROMPT：通过可插拔视觉提示在不改动各车编码器与协作堆栈的前提下，对异构协作感知特征进行快速对齐，实时、隐私友好，显著提升AP且参数极少。


<details>
  <summary>Details</summary>
Motivation: 现实协作感知中，车辆传感器、模型与训练域各异，导致共享特征空间不一致，融合与检测性能下降。现有方法常需重训融合/检测或为每模态定制解释器，扩展性差、依赖私有元数据且触及隐私。

Method: 1) 设计HeteroPROMPT：保持各代理编码器与协同融合/检测模块冻结，仅通过“视觉提示”(prompts)对BEV特征在通道与空间维度进行调制，实现异构特征到统一自车特征空间的快速对齐；2) 轻量学习式微调，模块化、按需路由；3) 为无元数据部署，引入自编码器学习紧凑统一表示，并从共享特征中提取模态线索，进行实时模态分类与路由；4) 整体实时、低算开销、隐私友好。

Result: 在OPV2V-H与V2XSet上，较SOTA异构CP方法提升AP，同时所需可训练参数数量低几个数量级；部署阶段的模态分类准确率>99.99%。

Conclusion: HeteroPROMPT提供可扩展、实用、隐私友好的异构协作感知方案：以提示对齐特征、冻结主干与协作堆栈、元数据无依赖，兼顾精度、效率与可扩展性。
...
TL;DR: 提出Eddeep，一个两阶段深度学习框架，在一次前向传播中同时校正dMRI中的涡流畸变与体间位移，达到与FSL Eddy相当的质量但显著加速推理。


<details>
  <summary>Details</summary>
Motivation: dMRI的EPI采集易受涡流引起的几何畸变影响，且畸变随梯度强度与方向变化，导致体间错配，进而偏置微结构分析。现有如FSL Eddy的最优方法依赖迭代预测-校正，计算开销大，限制大规模队列与临床实时性。

Method: 将问题分解为两阶段：(1) 监督图像翻译网络，将扩散加权与b0图像外观标准化以消除对配准不利的对比差异；(2) 无监督配准网络，在物理约束的二次畸变模型下，同时估计涡流畸变与体间头动参数，实现单次前向通过的校正。使用UK Biobank训练，并在UK Biobank与跨域Memodyn数据集评估。

Result: 在体间抖动、DKI残差、信号不规则度、互信息等多指标上，Eddeep取得与FSL Eddy相当的校正质量，同时显著降低推理时间；具备跨域泛化能力。

Conclusion: 深度学习可在无需迭代优化的前提下，实现准确高效的涡流畸变校正，促进大规模研究与临床流程加速；代码已开源（https://github.com/CIG-UCL/eddeep）。
...
TL;DR: MoSAIC提出一种面向人体局部部位的参考条件化运动风格迁移扩散框架，通过对内容与参考进行按解剖区域的解耦、独立根轨迹条件和可路由的局部参考，实现既保留原动作/时间/轨迹又能对选定身体部位施加参考风格的可控编辑；核心是“对齐干预监督”，用受控的局部变换构造同步参考与反事实目标，使所需的局部响应与需要被保留的部分在训练中可观测；实验显示在大规模冻结评测中，相比整身路由，局部遮罩路由显著降低保留区域误差和非目标泄漏，同时提升被选区域响应；继续训练对比也验证了该监督策略能提升选定目标响应与路由影响集中度。


<details>
  <summary>Details</summary>
Motivation: 现实编辑场景常需把某段手势或步态从一段或多段参考动作迁移到源动作的特定身体部位，同时严格保留原动作的内容、时序、根部轨迹和未选部位。然而现有数据集很少提供任意“内容—参考”的配对样本，且仅靠自重构训练的扩散模型容易复刻内容动作而忽视参考信号。因此需要一种在无大规模配对数据下也能有效学习“局部响应与全局保留”权衡的训练与模型机制。

Method: 提出MoSAIC：1) 特征因子化：将内容与参考按解剖区域分开编码；2) 根轨迹独立通道：通过单独条件保持整体位姿与路径；3) 路由机制：用户可把不同参考路由到不同身体部位；4) 核心训练“对齐干预监督”：用受控的局部变换构造时间对齐的参考与反事实目标，使被请求的区域变化和应被保留的部分在训练时显式可见，从而监督扩散模型在选区产生响应、在非选区保持不变；5) 采用潜空间扩散以稳定与高效训练。

Result: 在包含128段动作、896种路由条件的冻结评测中：相较整身路由，采用部位遮罩路由将保留区域误差从70.64 mm降至66.45 mm，将匹配噪声设置下的非目标泄漏从18.08 mm降至9.88 mm，同时保持对选定区域的正向响应；在等训练预算的继续研究中，保留对齐干预监督可带来8.8%的相对提升（选定目标响应）和2.0个百分点的提升（请求路由影响的集中度）。

Conclusion: MoSAIC在无需成对数据的条件下，通过对齐干预监督与部位路由，实现更好的“响应—保留”权衡，实现可选择、可控的局部人体运动编辑；相较整身路由，显著降低非目标影响与保留区域误差，并提升被选区域的风格迁移效果。
...
TL;DR: 论文研究MLLM在视觉为主任务中失败的原因，提出两套诊断：图像重建以检验视觉信息是否被编码，以及WhatIfVis基准以衡量模型在视觉证据与语言先验之间的依赖与可控性。发现：粗粒度属性在感知阶段已被编码，但模型在使用这些信息时失灵；SFT与激活补丁提高可控性并定位权衡层次；学习到的“视觉-先验”控制向量可无指令地提升依赖控制。


<details>
  <summary>Details</summary>
Motivation: MLLM常在视觉证据与语言先验冲突时出错，难以判断问题源于“看不见”还是“看到了却没用好”。需要拆分感知编码与后续利用两个环节，精确定位瓶颈并提出可控性改进方法。

Method: 1) 感知可用性：从冻结MLLM最终层图像token进行图像重建，以检测粗粒度视觉属性是否被保留。2) 语境敏感度：构建WhatIfVis数据集，覆盖时空、颜色、数量、大小、重量五类可由图像或先验回答的问题；比较在“使用/忽略视觉”指令下的行为，并用监督微调(SFT)与激活补丁定位“视觉vs先验”权衡层深。3) 训练并应用一个控制（steering）向量以连续调节对视觉与先验的依赖。

Result: (i) 粗粒度属性能从最终层图像token重建，说明视觉证据被保留；相应任务失败多属后感知阶段问题。(ii) 原始模型视觉语境敏感度不稳定；SFT显著提升可控性并跨域泛化，激活补丁进一步在不同架构中定位关键深度。(iii) 存在可学习的“视觉-先验”控制方向，无需显式意图指令也能提升对视觉依赖的可控性。

Conclusion: MLLM对粗粒度视觉属性的编码并非主要瓶颈，问题在于无法稳定控制对视觉证据与语言先验的依赖。通过SFT、激活补丁和控制向量，可在不改变感知表征的前提下显著改善可控性与鲁棒性。
...
TL;DR: 提出Zero-Fi：将Wi‑Fi信号特征与自然语言活动描述进行对比式对齐，实现零样本人类活动识别，能识别未见过的活动类别。


<details>
  <summary>Details</summary>
Motivation: 现有Wi‑Fi活动识别多为封闭集，需为每个目标类提供带标注的Wi‑Fi样本，难以识别未见活动；需要一种能泛化到新类别、减少标注依赖的方法。

Method: 设计对比式“信号-语言”对齐框架Zero‑Fi：1) 从互补的Wi‑Fi信号特征学习统一表示；2) 将其与自然语言活动描述的语义表示在共享嵌入空间内对齐；3) 通过跨模态对齐，在无该类标注Wi‑Fi样本和无需模型适配的情况下进行识别。

Result: 在大规模公开基准上，对被留出的活动类别实现有效的零样本识别，相比封闭集假设的方法展现出良好的泛化与可用性。

Conclusion: 跨模态信号-语言对齐能将Wi‑Fi感知从预定义类扩展到开放类，实现零样本活动识别，减少对标注数据与特定类别适配的依赖。

...
TL;DR: 对OASIS脑MRI配准，作者将“架构增强（仿射+可变形）”与“训练正则化损失”解耦做消融：正则化单独贡献了主要性能与形变控制提升；架构带来较小但互补收益。


<details>
  <summary>Details</summary>
Motivation: 以往深度学习图像配准工作常同时引入架构增强与正则项，难以判断各自贡献。临床部署受不合理形变（如折叠、极端位移）限制，需明确哪些手段真正减少不合理形变且不增加推理成本。

Method: 在OASIS数据集上训练四个变体：1) 基线3D U-Net+基础相似性损失；2) 基线+全套正则化；3) 仿射预配准+可变形架构+基础损失；4) 仿射架构+全套正则化。评估指标涵盖配准精度（MSE/NCC/SSIM）、形变质量（雅可比保持、位移统计、解剖可合理性评分）与计算成本。

Result: 正则化主导改进：相对MSE提升+21.3%（1.78%→2.16%，P<.001）、NCC改善+21.8%，最大位移从53.1降至0.51（-99.0%），推理时间几乎不变（-0.06%）。组合模型获得最大精度增益+25.8%（1.78%→2.24%），解剖可合理性0.596→0.930，推理时间+9.8%。梯度相关从0.742升至0.980。所有增强模型在合理形变约束下达成亚体素精度。

Conclusion: 在本设定下，训练正则化是主要驱动因素：几乎免费带来精度提升与形变控制；仿射架构提供次要但互补收益且代价可接受。对临床部署的关键障碍——不合理形变——实现了约99%的显著抑制。
...
TL;DR: 这是一篇关于扩散模型在图像、视频和3D三大视觉模态上的对抗攻击与防御研究的首个统一综述，提出任务中心的分类法，系统梳理方法、评测设置与基准，并提出开放挑战与未来方向。


<details>
  <summary>Details</summary>
Motivation: 扩散模型广泛可得且生成质量高，带来被滥用与安全风险；现有对抗研究分散在不同模态与任务，缺乏统一框架与系统性评估综述，因此需要汇总并规范化研究版图，帮助研究者理解威胁面与防御现状。

Method: 提出以“模态→攻防→生成任务”为主轴的任务中心化分类体系；在每个任务内按时间顺序组织工作；汇总各方法的评测设置（数据集、指标、基准），并进行对比分析；最后凝练开放问题与未来方向。

Result: 形成了覆盖图像、视频、3D三大模态的系统梳理：对攻击与防御方法进行分门别类和时间轴梳理；统一整理了主流评测流程、数据集与指标；明确当前研究在适用范围、鲁棒性、可转移性、计算开销与实际部署等方面的局限。

Conclusion: 建议面向跨模态与跨任务的统一评测基准、标准化威胁模型、面向实际部署的高效与可解释防御、以及与法律与伦理的协同规范；呼吁社区构建公开基准与复现实践，推动扩散模型安全研究走向系统化与可落地。
...
TL;DR: 论文提出一种频率感知的多任务加权方法（FTW），根据近期损失历史的低频能量占比来动态分配任务权重，在统一的YOLOv8多任务感知（目标检测、可行驶区域分割、车道线分割）中进行评估；结果显示在单次运行下FTW在部分指标上占优，但证据有限，未能证明超越基线或具备泛化性。


<details>
  <summary>Details</summary>
Motivation: 多任务统一感知可提升自动驾驶效率，但多任务联合优化受不同收敛速率、损失尺度与稳定性影响而困难。现有加权方法多基于损失幅值、不确定性、短期损失变化或梯度统计，缺乏从损失历史“频率结构”角度的信号。作者希望利用损失轨迹的稳定性代理来更合理地动态平衡任务。

Method: 提出Frequency-aware Task Weighting（FTW）：在每个任务上取近期损失窗口，做去均值后计算其低频能量占比，视为稳定性代理；低频占比高者赋予更大权重。将FTW集成到基于YOLOv8的统一感知框架（共享干网+三头：检测、可行驶区分割、车道线分割），并在两种训练配置下评测：静态全网络训练与渐进式冻结。与固定权重和基于不确定性的权重方法作对比。选择每次运行中验证损失最低的checkpoint作为汇报结果。

Result: 在Mapillary Vistas上，六种单次运行配置中：静态FTW获得最高的总体派生分数和车道mIoU；渐进式冻结+FTW获得最高检测mAP；而静态不确定性加权获得最高可行驶区mIoU。由于缺乏多次种子重复、单任务基线和FTW消融，这些排序仅具描述性。

Conclusion: 损失频率结构可作为动态任务加权的可行信号，FTW在该管线中显示潜力，但当前证据不足以证明其优于现有基线或具备更广泛泛化，需要多次复现实验、消融和更全面的对照来确证。

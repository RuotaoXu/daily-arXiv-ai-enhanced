...
TL;DR: 提出TRACE基准与FRM模块，用于处理火灾后物体的几何/材质/外观不可逆劣化；在多任务上显著缓解模型性能随劣化加剧而锐减的问题。


<details>
  <summary>Details</summary>
Motivation: 火灾后物体会发生物理与外观的不可逆转变，现有图像稳健性研究多聚焦于成像层面的扰动，缺乏对物体本体结构劣化的系统评测与方法；需要可评估与提升在此情境中定位、还原与理解能力的基准与方法。

Method: 1) 构建TRACE：21.4K真实图像支撑的合成场景，含499个物体身份、189类，提供从完好到劣化的配对进程；设置五个任务：劣化目标检测、完好态检索与还原、原始材质还原、完好描述生成、功能推理。2) 观察现有检测/多模态模型在劣化加剧下性能陡降。3) 提出可插拔的Feature Recovery Module（FRM）：冻结主干，仅用配对特征监督，将劣化编码器特征映射到与完好态对齐的表征，用于提升检测与VLM对象级任务及CLIP/SigLIP2特征恢复。

Result: 基线退化显著：如RF-DETR mAP相对下降71%，InternVL3.5检索R@1由93.85降至28.11。引入FRM后，在场景级检测、特征恢复与四个对象级VLM任务上均有提升，且劣化越重收益越大：跨宿主与劣化等级，平均相对提升为：检索+12.5%，材质恢复+20.1%，描述生成+13.2%，功能推理+12.4%。

Conclusion: TRACE填补了物体结构性劣化场景的评测空白；FRM在不改动宿主模型的前提下有效将劣化表征对齐到完好态，全面提升多任务性能，尤其在严重劣化下更显著。
...
TL;DR: 论文提出“读时内存替换”方法，直接测试视频模型记忆收益是否来自被检索内容本身，区分记忆是否仅提供通用表示支持、广义上下文，或依赖精确情节内容。在多种视频世界模型与分割模型上，作者发现：有时与任务无关的“身份无关”控制值即可恢复大部分收益（表明表示修复/支持作用），而在需要精确空间记忆的任务中，更换为错误记忆会大幅崩溃性能，证明强内容依赖。


<details>
  <summary>Details</summary>
Motivation: 现有“是否使用记忆”的消融只能说明记忆是否有用，无法回答“收益是否来自正确的被检索内容”。作者动机是建立可操作的实验范式，直接检验记忆收益对特定记忆内容的依赖度（memory specificity），从而解释记忆机制：是通用表示修复、广义上下文，还是精确情节内容。

Method: 提出读时内存替换（read-time substitution）：在前向计算中，仅替换被读取的内存条目，保持其他计算不变。设置多类替换源：1）身份无关/与评测无关的控制值；2）同轨迹内“错误记忆”；3）跨轨迹/跨生物群落（biome）的供体；4）与训练中观察到的记忆表征距离可控的剂量响应（dose response）。在多个数据集（Ego-Exo4D、7-Scenes、TUM、DAVIS、MOSEv2）和模型（冻结的视频世界模型、WorldMem、SAM 2）上度量PSNR、分割区域/边界分数等指标的恢复比例。

Result: - 冻结视频世界模型：身份无关控制即可在Ego-Exo4D与7-Scenes上几乎完全恢复收益，在TUM上约70%。在Ego-Exo4D的剂量响应中，替换值从接近训练记忆表征到远离时，恢复率由102%降至1%，支持“表示修复”解释。
- WorldMem：对内容有分级依赖；同轨迹错误记忆相对零内容可恢复94.1%的PSNR收益；跨轨迹/跨biome的供体仍可恢复43.7%。
- SAM 2（视频分割）：对精确内容强依赖；DAVIS上以“有效但错误”的空间记忆替换会使区域/边界均分从0.926降至0.182；MOSEv2再出现场景从0.459降至0.000。

Conclusion: 记忆带来的性能提升并不总依赖于检索到的精确内容：有时仅需通用表示支持或更广上下文即可显著受益，但在需要精确空间/情节信息的任务中，内容正确性至关重要。读时内存替换提供了区分这些机制（表示修复、上下文支持、精确情节内容）的直接工具与实证证据。
...
TL;DR: 论文在多源冻结大模型组合进行少样本3D分割时，比较“单一Top-1类别投票”与“保留分布并融合”的差异，发现保留多候选语义再融合显著提升HM IoU，揭示过早语义坍缩是信息瓶颈。


<details>
  <summary>Details</summary>
Motivation: 传统分类器融合多在分数级或硬投票层面，缺少对“在模型交互前丢失多少语义信息”的定量诊断；在当下流行的异构、冻结的基础模型组合场景（3D分割、跨模态/稀疏密集源）中，这一问题尤为关键。

Method: 提出同输入、语义保留干预：固定两类来源（稠密RegionPLC与稀疏跨视角SAM3）的权重、掩码、几何、词表与融合规则，仅通过匹配的top-k梯度调节在交互前保留的候选语义数量；比较top-1（硬决策）与完整分布融合（或其他运算如max/几何池化），并进行权重扫描、源替换（GroundingDINO–SAM2.1）与校准诊断。

Result: 在ScanNet200上，top-1得28.47 HM IoU；完整分布融合达34.87 HM，提升+6.40（95% CI [+5.24,+7.64]）。在ScanNet++上复现：23.02 vs 26.50，+3.48（95% CI [+1.64,+5.93]）。在稀疏源权重0.3–0.7范围内稳定；替换源后完整保留使HM从14.77单调增至18.75；不同融合算子亦优于top-1；两源呈相反失配校准，校准后仍保留优势。

Conclusion: 在异构冻结模型组合中，过早将多源语义坍缩为单类会形成可复现的信息瓶颈；保留一小撮合理候选并做分布级融合即可回收大部分信息，且对权重、算子与源更换具有稳健性，校准并非主要因素。
...
TL;DR: 该论文在HSI-Road数据集上新增六类地表语义标注，并评估多种输入（高分辨率RGB、配准低分辨率RGB、NIR、以及RGB与NIR通道堆叠）对六种分割模型的影响：高分辨率RGB总体最好，但在匹配低分辨率下，RGB+NIR通道堆叠普遍优于单独的NIR和多数情况下优于配准RGB，尤其提升“水体”类别，强调空间分辨率重要性与NIR对RGB的互补性。


<details>
  <summary>Details</summary>
Motivation: 原HSI-Road仅有二值掩码且缺乏具体地表类别标注，难以开展细粒度道路场景理解与多模态（RGB/NIR）融合研究。作者希望提供可用于评估多模态与分辨率影响的标准基准与标注。

Method: 1) 手工构建六类地表语义标注（背景、沥青、混凝土、土、 水、草）。2) 提出RGB到NIR的配准流程并提供对应标注。3) 在四种输入设置（高分辨率RGB、配准低分辨率RGB、NIR、以及RGB与NIR通道堆叠）下，对六种主流语义分割模型进行训练与评测。4) 使用每类与平均IoU、F1作为指标，比较空间分辨率降低与多模态融合的影响。

Result: 高分辨率RGB整体最佳，但其像素量为匹配分辨率输入的12倍。在192×384的匹配分辨率下，RGB+NIR通道堆叠在六个模型中均优于NIR，并在六个模型中的五个优于配准RGB；对“水体”类别提升最稳健。

Conclusion: 空间分辨率对分割性能影响显著；在匹配低分辨率条件下，NIR能为RGB提供互补信息，RGB与NIR的通道堆叠通常优于单独模态，特别有利于水体识别。

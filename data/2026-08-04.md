...
TL;DR: 提出ELECTRIC：一种将证据学习与物理引导的CT迭代重建融合的贝叶斯闭环框架。用证据神经网络给出图像先验与可校准的不确定性，并在Poisson加权的MAP更新中转化为自适应精度场，实现“图像—证据—精度—重建”的闭环。实验显示大幅降误、误差可预测、测量一致性恢复，且对先验强度失配更鲁棒。


<details>
  <summary>Details</summary>
Motivation: 传统CT重建（如FBP或固定先验的MAP）要么忽略物理噪声与数据一致性，要么难以处理先验不确定性的空间变化；深度先验虽有效，但可信度难量化、对先验强度敏感，可能在失配时退化。需要一个能将可解释的不确定性估计与物理一致的重建过程有机结合、并能自适应调整先验影响的框架。

Method: 构建物理引导的贝叶斯MAP重建，将证据神经网络输出的图像先验均值与表征认知不确定性的代理量（如Normal-Inverse-Gamma的参数）转化为空间自适应的精度场，并嵌入Poisson加权的MAP更新中；形成闭环：网络给出提案与不确定性→转换为精度→迭代物理一致更新→再评估。含理论分析与两类模拟：1) 机制验证（透明替代估计器）；2) 可行性研究（训练NIG证据网络驱动全闭环）。

Result: 在AAPM Mayo低剂量CT切片的留出病人上：学习到的先验均值相对FBP将重建误差降低约70%；所学认知不确定性与误差正相关，支持选择性信任；物理引导更新恢复测量一致性；自适应精度重建匹配或优于经验证集调参的固定先验，并且对先验强度设定失配更稳健。

Conclusion: ELECTRIC实现了证据先验与物理一致迭代重建的闭环，显著提升重建精度与稳健性，并提供误差可预测的不确定性表征。未来关键方向是形式化的不确定性校准与联合训练以进一步提升性能与一致性。
...
TL;DR: 提出NR-CFM：一种在仅有单次受噪观测下学习干净数据分布的生成模型，针对高斯噪声给出闭式端点校正，对更一般的高斯协方差学得数据驱动校正，较NR-GAN多数情形更优，并在高噪声（SNR低至0.001）科学成像中生成可信图像，性能与Ambient Diffusion在高噪区间相当。


<details>
  <summary>Details</summary>
Motivation: 科学成像常只能获得含噪测量，干净参考昂贵或不可得；直接用含噪数据训练会复现噪声，需从带噪样本直接学习干净分布。希望在稳定训练与高效采样框架下解决这一“无干净标注”的学习问题。

Method: 基于Conditional Flow Matching (CFM) 的无条件生成：1) 针对白噪（加性独立同分布高斯），推导闭式的“干净端点”校正，使训练目标对噪声鲁棒；2) 针对具有一般协方差结构的高斯噪声，学习数据驱动的校正项；3) 以简单回归式CFM目标进行稳定训练，并高效采样。

Result: 在多种腐蚀情形下，NR-CFM多数情况下优于NR-GAN；在高噪声条件下与Ambient Diffusion竞争；在SNR低至0.001的科学数据上，仍能从严重退化观测生成合理的粒子图像。

Conclusion: NR-CFM能在仅有一次受噪观测的设定下，稳健地从高斯噪声污染数据中学习干净数据分布，兼具理论校正与数据驱动适配，表现优于或不逊于现有无参考生成方法，适用于极低信噪比科学成像场景。
...
TL;DR: 提出PhysAgent：一种在推理阶段进行多代理候选验证的rPPG心率融合框架，用轻量MLLM做条件与可靠性推理、确定性生理校验与可复现实数融合，提升跨数据集与域的稳健性，避免直接MLLM预测与无约束集成的不一致与不可复现。


<details>
  <summary>Details</summary>
Motivation: rPPG从人脸视频估计心率，但信号极弱，易受运动、光照、遮挡、肤色/皮肤外观差异、设备噪声影响。单一模型直接预测心率或脉搏常给出相互冲突但各自看似合理的结果，缺乏机制来化解冲突与评估可信度。需要一种既不重新训练基模型、又能在推理时利用多源候选并进行生理一致性验证与可复现实数融合的方法。

Method: 构建PhysAgent：将多个强rPPG基估计器的输出视为“生理假设”。使用轻量4B多模态大模型Qwen3-VL-4B作为推理代理，围绕视频条件（运动、光照、遮挡等）、信号质量与候选分歧开展多代理推理，提出融合建议；随后用确定性的生理验证器检查融合建议的生理一致性；最后以可复现的数值融合流程生成最终心率。不训练新基模型，也不让MLLM直接给心率。

Result: 在多个公开rPPG基准与不同源域设置下，PhysAgent较现有方法提升了融合的稳定性与可靠性；相较直接MLLM预测或不受约束的集成融合，避免了不可复现与生理不一致问题。

Conclusion: 将多基估计器输出转化为待验证的生理假设，结合轻量MLLM的条件/可靠性推理与确定性生理校验及数值融合，可在无需额外训练的前提下稳定提升rPPG心率估计的稳健性与可复现性。
...
TL;DR: 提出SafeBuild-Bench基准与GEMS数据选择流程，评估并提升多模态大模型在真实施工安全场景（跨时间与工地点变）中的隐患识别能力；结果显示现有模型仍不可靠，最佳得分约60。


<details>
  <summary>Details</summary>
Motivation: 现有多模态模型多在整洁数据上做物体识别，难以处理施工现场中长尾、冗余、跨时间与场地变化的真实隐患（如脚手架无护栏边缘作业）。需要一个贴近工业现实、可扩展标注与验证的评测基准与数据筛选方法。

Method: 1) 构建SafeBuild-Bench：从10万+工业图文档案中挖掘，筛得3000+经专家核验图像与3314任务，涵盖多选隐患识别与自由文本描述，并保留时间与场地变化元数据。2) 提出GEMS管线：以“代理模型困惑信号+图结构多样性”从冗余流中挑出信息量大的候选，实现可扩展专家验证；并在公开指令微调数据上做子集选择以保留鲁棒性表现。3) 给出评测脚本与代码。

Result: 在公开指令调优数据上，经GEMS选择的小规模子集仍能保持面向鲁棒性的性能；在SafeBuild-Bench上，现有MLLM最佳总体分仅约60，显示与可靠施工安全理解仍有显著差距。

Conclusion: SafeBuild-Bench为评估MLLM在真实施工安全场景下的能力提供了首个元数据驱动基准，GEMS提升了从冗余档案中选取高价值样本并扩展专家核验的可行性；当前模型性能不足，基准与代码的开源有望推动后续研究与更可靠的现场安全智能。
...
TL;DR: 基于两大公开数据集（共3644例）对多种3D CT基础模型进行无复发生存预测基准评测，发现外部验证显著掉点；影像+临床融合仍最准，但跨中心泛化困难。


<details>
  <summary>Details</summary>
Motivation: 3D CT基础模型兴起，被视为可替代易受采集协议影响、重现实验差的传统放射组学；但其表征在不同临床场景的可迁移性与是否需要下游任务适配尚不清楚。

Method: 在头颈癌两项公开队列上，对多种3D CT基础模型进行基准评测，任务为复发无病生存预测；系统比较了不同模型适配策略（微调/线性探针等）与模态融合方案（影像与临床数据融合）。采用外部验证评估跨分布泛化。

Result: 多数模型在外部队列上性能明显下降，难以找到对不同影像分布稳定泛化的特征；将影像特征与临床信息融合可获得最佳预后预测性能，但仍未实现“到处都好用”的一致泛化。

Conclusion: 当前3D CT基础模型在跨中心泛化方面仍存在显著挑战；需要任务特定的适配与与临床数据融合以提升性能，但实现普适泛化仍任重道远。
...
TL;DR: 研究探索将混合量子卷积神经网络（QCNN）用于地球观测中火山云卫星图像分类，并与经典模型对比。


<details>
  <summary>Details</summary>
Motivation: 火山云监测对航空安全、灾害评估与气候研究至关重要，但因其与气象云相似、爆发特征多变及地球同步卫星光谱采样粗糙，准确检测困难。量子计算与QML或可通过叠加与纠缠提升复杂光谱-空间特征提取能力，值得评估其在EO中的增益。

Method: 在经典CNN框架中插入量子层，构建混合QCNN；设计2与4量子比特两种量子子网络，对SEVIRI卫星图像数据集进行分类（含火山灰、SO2与混合成分的火山云，以及非火山背景）；将两种QCNN与纯经典架构进行性能对比。

Result: 两种量子变体在给定数据集上完成分类任务，并产生可与经典模型进行对比的性能结果（摘要未给出具体数值或统计显著性）。

Conclusion: 混合QCNN在火山云分类任务上具备潜力，值得与经典模型并行评估；实际优势与泛化能力仍需更多实证与更详细的量化结果来确认。
...
TL;DR: 提出一个可审计的三分数据集分析与分割流程，利用空间-强度-时间联合特征的无监督聚类与分层抽样，显著降低纵向医学影像数据在训练/验证/测试间的协变量偏移与时间不均衡，实现稳定、可复现的队列划分。


<details>
  <summary>Details</summary>
Motivation: 随机划分小型、纵向临床影像队列易造成跨子集的强度分布偏移与随访时间不均衡，导致模型在分布外评估、性能不稳且不可复现；需要一种系统、可审计的方法来表征并控制这些偏移。

Method: 提出“Tripartite Dataset Analytics Framework”：1) 评估空间网格完整性；2) 提取多参数强度指纹；3) 建模纵向时间轨迹，量化重尾分布与不规则随访。基于此，定义无监督的队列平衡SOP：在标准化的六维强度-时间联合特征空间上，用肘部法选择K的K-means聚类，并在簇内按比例分层抽样，将样本分配到训练/验证/测试以保持跨子集一致性。通过蒙特卡洛多种随机种子与拆分配置进行压力测试与稳定性检验。

Result: 在一组对比增强T1加权脑MRI纵向队列（N=149）上，相比随机洗牌的最大跨子集强度偏差34.1%，该协议将其降至<2.1%，并使随访时间间隔围绕总体均值紧密对齐；多次随机种子与三种划分设置下，这种对齐保持紧密界限，而随机划分表现出显著波动。

Conclusion: 该协议提供了一种可复现、可推广的队列工程流程，能在可变长度的纵向临床影像工作流中显著减小协变量漂移与时间不均衡，提升下游深度学习评估的可靠性与公平性。
...
TL;DR: 提出一个多模态机学习框架，用于考古数字化流程中的标定监控、质量评估与自适应采集支持，统一处理摄影测量3D、超光谱、XRF与拉曼数据，通过特征表示+分类/异常检测+可解释AI，实现跨模态的采集质量判别与纠偏建议。


<details>
  <summary>Details</summary>
Motivation: 考古多模态采集链路复杂、设备和条件多变，传统仪器级标定不足以保证跨模态数据的一致性与可集成性；需要一个算法层评估采集是否统计一致、物理可信、并能指导现场纠偏与资源分配。

Method: 为每种传感模态构建结构化特征空间（几何、光谱、空间、统计特征），结合确定性质量指标与统计描述；用监督分类与无监督异常检测判别“可用/问题”采集；利用XAI解释退化成因；将特征偏差映射到可操作的采集纠偏策略，实现自适应和资源感知采集。

Result: 在多模态考古数据集上，框架能捕捉有意义的采集变异与退化模式（如重建伪影、光照不一致、光谱畸变、探测器不稳、基线漂移、低信噪），实现稳健的跨模态质量评估与问题定位。

Conclusion: 该统一多模态与XAI驱动的方法可作为仪器标定之上的算法层，可靠地评估与解释采集质量，并为现场提供反馈与资源优化，提升下游多模态融合的可用性与鲁棒性。
...
TL;DR: 提出KGR：在原生k空间进行连续表征，用共享几何的Gabor‑Gaussian基+低秩投影+频率自适应拟合，提升加速MRI重建质量。


<details>
  <summary>Details</summary>
Motivation: 现有方法在离散采样格点上插值或加先验，只重建离散频率系数，难以显式建模底层连续信号，且对多线圈内在结构（相位平滑、线圈冗余）刻画不足。需要在k空间直接进行连续表征并与结构化低秩约束结合。

Method: 以Gabor‑Gaussian原语参数化连续k空间信号，多个线圈共享空间几何以保留跨线圈相关；先进行连续拟合，再将估计的表示投影到低秩流形以满足由相位平滑与线圈冗余引出的代数约束；采用频率自适应的拟合策略，针对不同k空间区域的异质特性优化参数与权重。

Result: 在多数据集、多采样方案上，相比代表性基线在指标与视觉质量上均有一致提升，证明方法泛化与稳健性。

Conclusion: 在原生k空间进行显式连续参数化，并结合结构化低秩约束，是融合连续信号建模与多线圈结构先验的有效框架，可提升加速MRI重建的保真度与视觉质量。

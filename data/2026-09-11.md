...
TL;DR: 提出GraphemeNet：以结构先验提升多脚本手写字符识别，在更少参数下达到或超越多项基线。


<details>
  <summary>Details</summary>
Motivation: 非拉丁手写字符识别长期欠研究，主流把它当通用图像分类，依赖大模型去隐式学习笔画与结构，效率与泛化受限。作者主张：显式编码脚本几何规律（结构先验）可同时提升准确率与参数效率。

Method: 统一多脚本架构GraphemeNet，由两个正交二元轴控制：1）通过PSI（Persistent Scaffold Injection）在每个编码阶段注入脚本特定的非对称卷积“笔画脚手架”，作为加权残差，持续锚定特征到脚本几何；2）根据任务是否需要空间关系推理，在全局平均池化+门控融合与跨尺度注意力+Stroke Topology Module（STM）之间切换。所有脚本共用线性胶囊路由（LCR，O(n)）。每种脚本仅更换脚手架与解码器拓扑。

Result: 在8种书写系统、14个基准上泛化良好，统一框架下以更少参数稳定匹配或超越已发表基线；当需要空间关系时，跨尺度注意力+STM带来更强区分力。

Conclusion: 结构先验效率原则成立：显式编码脚本几何与拓扑到网络结构，可在多脚本HCR中实现更高准确率与更少参数；GraphemeNet作为统一架构具备可迁移性，仅需更换脚手架和解码器拓扑即可适配新脚本。
...
TL;DR: AcFlow 是一种在推理阶段对 DiT 进行零改动控制的“速度场”方法：用文本概念条件化的向量场在中间层激活上做连续形变，从而连续调节风格强度并抑制不想要的概念；无需为单个概念单独拟合，且对未见概念有泛化。


<details>
  <summary>Details</summary>
Motivation: 直接用提示词控制 DiT 的风格强度与概念抑制往往不稳定、粒度粗且易失败；现有方法缺少在不改动底座模型的前提下、可连续调节且能跨概念泛化的控制机制。

Method: 冻结基础 DiT，仅在推理时引入一个“概念条件化的速度场”，对中间层图像 token 激活作时间积分式的运输（由积分时长作为连续控制参数）。该速度场随 token 与其激活状态变化，任务族内参数共享，可用细粒度文本描述指定干预，无需逐概念拟合，对未见概念可泛化。

Result: 在风格控制任务中，在高风格对齐区间实现最佳风格-内容权衡；在固定点上达到风格/内容对齐 0.5365/0.2860，优于最高风格对齐的基线 0.4397/0.2684。质化结果展示对多样“不想要概念”的有效抑制，包含直接提示失败的情形。分析表明速度场为自适应控制：更新方向随 token 与其激活而变。

Conclusion: AcFlow 以推理期、激活级别的速度场实现对 DiT 的连续可控干预，在不改动底座的前提下兼顾风格与内容、抑制不良概念，并对未见概念泛化；适合作为通用的文本条件可控生成控制器。
